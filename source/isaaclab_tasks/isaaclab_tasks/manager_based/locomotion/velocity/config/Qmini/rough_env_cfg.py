...
# All rig        # scene
        ...
        # actions - 增加動作範圍以支持更大步伐
        self.actions.joint_pos.scale = 0.8  # 增加到0.8以允許更大的關節運動reserved.
#
...
@configclass
class QminiRoughRewards(RewardsCfg):
    ...
    dof_pos_limits = RewTerm(
        func=mdp.joint_pos_limits, weight=-1.0, params={"asset_cfg": SceneEntityCfg("robot", joint_names="ankle_pitch_.*")}
    )
...
class QminiRoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    """Qmini rough environment configuration."""

    rewards: QminiRoughRewards = QminiRoughRewards()
...
@configclass
class QminiRoughEnvCfg_PLAY(QminiRoughEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
        ...
        self.commands.base_velocity.ranges.lin_vel_x = (1.0, 1.2)  # 適度提高固定前進速度
        self.commands.base_velocity.ranges.lin_vel_y = (-0.2, 0.2)  # 適度允許側向移動
        self.commands.base_velocity.ranges.ang_vel_z = (-1.5, 1.5)  # 適度增加轉向幅度
