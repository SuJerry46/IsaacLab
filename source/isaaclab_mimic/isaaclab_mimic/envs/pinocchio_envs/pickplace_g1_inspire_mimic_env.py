...
class PickPlaceG1InspireMimicEnv(ManagerBasedRLMimicEnv):
    ...
    def get_robot_eef_pose(self, eef_name: str, env_ids: Sequence[int] | None = None) -> torch.Tensor:
        ...
        """
        env_ids_slice = slice(None) if env_ids is None else env_ids

        eef_pos_name = f"{eef_name}_eef_pos"
        eef_quat_name = f"{eef_name}_eef_quat"

        target_wrist_position = self.obs_buf["policy"][eef_pos_name][env_ids_slice]
        target_rot_mat = PoseUtils.matrix_from_quat(self.obs_buf["policy"][eef_quat_name][env_ids_slice])
    ...
    def target_eef_pose_to_action(
        ...
        Args:
            ...
            action_noise_dict: Optional dictionary of action noise for each end-effector.
            env_id: Environment index (unused but required for interface).

        Returns:
            torch.Tensor: Action tensor to be sent to the environment.
        """
    ...
    def action_to_target_eef_pose(self, action: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        # Left hand pose (first 7 dimensions: 3 position + 4 quaternion)
        ...
        # Right hand pose (next 7 dimensions: 3 position + 4 quaternion)
    ...
    def actions_to_gripper_actions(self, actions: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        """
        # G1 has 24 hand joints total (12 per hand)
        # The hand actions come after the 14 dimensions for the two end-effector poses
