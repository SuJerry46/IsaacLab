...
"""
This script demonstrates an interactive demo with the Qmini rough terrain environment.

.. code-block:: bash

    # Usage
    ./isaaclab.sh -p scripts/demos/qmini_locomotion.py
...
parser = argparse.ArgumentParser(
    description="This script demonstrates an interactive demo with the Qmini rough terrain environment."
)
...
from isaaclab_tasks.manager_based.locomotion.velocity.config.Qmini.flat_env_cfg import QminiFlatEnvCfg

TASK = "Isaac-Velocity-Flat-Qmini-v0"
RL_LIBRARY = "rsl_rl"


class QminiFlatDemo:
    """This class provides an interactive demo for the Qmini flat terrain environment.
    It loads a pre-trained checkpoint for the Isaac-Velocity-Flat-Qmini-v0 task, trained with RSL RL
    ...
    * ESC: exit current third-person view"""
    ...
    def __init__(self):
        ...
        # create environment config for play mode
        env_cfg = QminiFlatEnvCfg()
        ...
        # load previously trained model (try to find a checkpoint)
        try:
            checkpoint = get_published_pretrained_checkpoint(RL_LIBRARY, TASK)
            if checkpoint is None:
                # Try to find local checkpoint
                import glob
                checkpoint_paths = glob.glob("logs/rsl_rl/qmini_*/*/model_*.pt")
                if checkpoint_paths:
                    checkpoint = max(checkpoint_paths, key=os.path.getmtime)  # Get the latest checkpoint
                    print(f"[INFO] Using local checkpoint: {checkpoint}")
                else:
        ...
        if checkpoint:
            ...
            print("[INFO] Using random policy (no checkpoint loaded)")
    ...
    def set_up_keyboard(self):
        ...
        # Movement parameters optimized for Qmini
        T = 0.8  # Forward velocity (reduced for stability)
        R = 0.4  # Rotation velocity (reduced for stability)
        
        self._key_to_control = {
            "UP": torch.tensor([T, 0.0, 0.0, 0.0], device=self.device),
            "DOWN": torch.tensor([0.0, 0.0, 0.0, 0.0], device=self.device),
            "LEFT": torch.tensor([T, 0.0, 0.0, R], device=self.device),     # 左转：正角速度
            "RIGHT": torch.tensor([T, 0.0, 0.0, -R], device=self.device),   # 右转：负角速度
        ...
        if event.type == carb.input.KeyboardEventType.KEY_PRESS:
            ...
            if event.input.name in self._key_to_control:
                if self._selected_id is not None:
                    ...
                    print(f"[INFO] Robot {self._selected_id}: {event.input.name} pressed")
            ...
            # C key swaps between third-person and perspective views
                ...
                if self._selected_id is not None:
                    if self.viewport.get_active_camera() == self.camera_path:
                        self.viewport.set_active_camera(self.perspective_path)
                        print("[INFO] Switched to perspective view")
                    else:
                        self.viewport.set_active_camera(self.camera_path)
                        print("[INFO] Switched to third-person view")
    ...
    def update_selected_object(self):
        """Determines which robot is currently selected and whether it is a valid Qmini robot.
        ...
        if len(selected_prim_paths) == 0:
            ...
            if len(prim_splitted_path) >= 4 and prim_splitted_path[3][0:4] == "env_":
                ...
                if self._previous_selected_id != self._selected_id:
                    self.viewport.set_active_camera(self.camera_path)
                    print(f"[INFO] Selected Qmini robot {self._selected_id}")
                self._update_camera()
            else:
                print("The selected prim was not a Qmini robot")
...
def main():
    """Main function."""
    print("=" * 80)
    print("Qmini Interactive Demo")
    print("=" * 80)
    print("Controls:")
    print("  - Click on a robot to select it")
    print("  - UP Arrow: Move forward")
    print("  - DOWN Arrow: Stop")
    print("  - LEFT Arrow: Turn left while moving forward")
    print("  - RIGHT Arrow: Turn right while moving forward")
    print("  - C: Switch camera view")
    print("  - ESC: Deselect robot")
    print("=" * 80)
    
    demo_qmini = QminiFlatDemo()
    obs, _ = demo_qmini.env.reset()
    
    while simulation_app.is_running():
        # check for selected robots
        demo_qmini.update_selected_object()
        with torch.inference_mode():
            action = demo_qmini.policy(obs)
            obs, _, _, _ = demo_qmini.env.step(action)
            # overwrite command based on keyboard input
            obs[:, 9:13] = demo_qmini.commands
